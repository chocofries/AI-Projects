...
void InputManager::SpecialKeyDown(int _key, int x, int y)
{
	...
	case GLUT_KEY_LEFT:
	{
		if (arrowState[DIR_LEFT] == KEY_UP)
		{
			arrowState[DIR_LEFT] = KEY_DOWN_BEGIN;
		}
		else
		{
	...
	case GLUT_KEY_RIGHT:
	{
		if (arrowState[DIR_RIGHT] == KEY_UP)
		{
			arrowState[DIR_RIGHT] = KEY_DOWN_BEGIN;
		}
		else
		{
	...
	case GLUT_KEY_UP:
	{
		if (arrowState[DIR_UP] == KEY_UP)
		{
			arrowState[DIR_UP] = KEY_DOWN_BEGIN;
		}
		else
		{
	...
	case GLUT_KEY_DOWN:
	{
		if (arrowState[DIR_DOWN] == KEY_UP)
		{
			arrowState[DIR_DOWN] = KEY_DOWN_BEGIN;
		}
		else
		{
...
{
	mouseWheelDir = 0;
	mouseState[MOUSE_RIGHT] = KEY_UP;
	mouseState[MOUSE_LEFT] = KEY_UP;
	arrowState[DIR_UP] = KEY_UP;
	arrowState[DIR_DOWN] = KEY_UP;
	arrowState[DIR_LEFT] = KEY_UP;
	arrowState[DIR_RIGHT] = KEY_UP;
