...
void Level::Initialise(Game* _myGame, ShaderLoader* _shaderloader, TextureLoader* _textureloader, InputManager* _inputManager, int _rows, int _columns)
{
	...
	//Init Text
	Texter* title = new Texter("SPACE: Find Brightest Star       UP/DOWN: Simulated Annealing Temperature       LEFT/RIGHT: Search Radius","Assets/Fonts/ComicSans.ttf", glm::vec3(-500.0f, -400.0f, 2.0f), shaderLoader, textureLoader, levelCamera);
	...
	for (unsigned int i = 0; i < columns; i++)
	{
		for (unsigned int j = 0; j < rows; j++)
		{
			...
			hillclimber->SetTileGrid(i, j, temp); //Add to grid of tiles
...
void Level::Update()
{
	...
	std::string strIterations = std::to_string(hillclimber->GetSearchTemp());
	const std::string str1 = "Search Radius: " + strRadius;
	const std::string str2 = "Temperature: " + strIterations;
	text1->SetText(str1);
	text2->SetText(str2);
}
...
void Level::ProcessKeyInput()
{
	if (!hillclimber->GetIsFinding())
	{
		...
		{
			int prev = hillclimber->GetSearchTemp();

			if (prev - 10 >= 1)
			{
				hillclimber->SetSearchTemp(prev - 10);
			}
		}
...
HillclimberSA * Level::GetPathfinder()
{
	return hillclimber;
}
